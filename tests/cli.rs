...
#[test]
fn a_configuration_naming_a_removed_rule_says_so() {
    let directory = tempfile::tempdir().unwrap();
    fs::write(
        directory.path().join("straitjacket.toml"),
        "[facts]\nexact-clones = true\n",
    ...
    let output = binary()
        .arg("--config")
        .arg(directory.path().join("straitjacket.toml"))
        .current_dir(directory.path())
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(2));
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("exact-clone"), "{stderr}");
    assert!(stderr.contains("no longer has"), "{stderr}");
}

#[test]
fn skipping_a_removed_rule_says_so() {
    let output = binary()
        .args(["--no-config", "--skip", "error-discard"])
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(2));
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(stderr.contains("error-discard"), "{stderr}");
    assert!(stderr.contains("no longer has"), "{stderr}");
}
